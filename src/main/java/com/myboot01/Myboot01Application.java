...
@SpringBootApplication  // 이 클래스가 스프링 부트로 만든 애플리케이션의 시작 클래스임을 의
@ComponentScan(basePackages = {"com.myboot01","com.ruby" }) // 다른 패키지에 있는 클래스까지 등록하기 위한 어노테이션
public class Myboot01Application {    //스프링 부트로 만든 애플리케이션을 실행하기 위해서 메인클래를 작성, 클랫스명은 프로젝트이름+Application.java 형태로 메일 클래스가 작성됨, 별도 메인 클래스를 작성하지 않고 이 프로그램만 실행하면 된다.
